...
$(function(){
	Highcharts.chart('container1', {
		...
		tooltip: {
			...
			pointFormat: '<span style="color:{point.color}">{point.color}</span>: <b>{point.y:.2f}%</b> of total<br/>'
		},
		series: [{
			name: '영화명',
			colorByPoint: true,
...
<div class="card shadow mb-4">
	<div class="card-header py-3">
		<h6 class="m-0 font-weight-bold text-primary" >좌석 점유율</h6>
	</div>
		<div id="root">
				...
				render() {
					const html = this.state.data_json.map((m)=>
						<tr onClick={this.handleClick.bind(this, m)} >
							<td>{m.movieNm}</td>
							<td>{m.totSeatCnt}</td>
							<td>{m.totSeatCntRatio}</td>
							<td>{m.rankOldAndNew=="NEW" ? m.rankOldAndNew : m.rankInten}</td>
						</tr>
					);
					return (
						<div className="row" >
